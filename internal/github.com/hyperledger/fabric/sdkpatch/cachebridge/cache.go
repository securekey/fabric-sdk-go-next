...
func getKeyPairFromSKI(keyPairCacheKey *KeyPairCacheKey) (*pkcs11.ObjectHandle, error) {
	...
	if err := keyPairCacheKey.Mod.FindObjectsInit(keyPairCacheKey.Session, template); err != nil {
		logger.Errorf("Failed to FindObjectsInit [session: %d] from [ski: %x]", keyPairCacheKey.Session, keyPairCacheKey.SKI)
		return nil, err
	}
	...
	if err != nil {
		logger.Errorf("Failed to FindObjects [session: %d] from [ski: %x]", keyPairCacheKey.Session, keyPairCacheKey.SKI)
		return nil, err
	}
	if err = keyPairCacheKey.Mod.FindObjectsFinal(keyPairCacheKey.Session); err != nil {
		logger.Errorf("Failed to FindObjectsFinal [session: %d] from [ski: %x]", keyPairCacheKey.Session, keyPairCacheKey.SKI)
		return nil, err
	}

	if len(objs) == 0 {
		logger.Errorf("Objects empty [session: %d] from [ski: %x]", keyPairCacheKey.Session, keyPairCacheKey.SKI)
